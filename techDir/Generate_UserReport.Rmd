---
title: "`r paste0('Data analysis report of run ', commandArgs(trailingOnly=T)[3], ', library ', commandArgs(trailingOnly=T)[4])`"
author: "`r paste0('by Alithea Genomics for ', commandArgs(trailingOnly=T)[1], ' (PI: ', commandArgs(trailingOnly=T)[2], ')' )`"
date: "`r format(Sys.time(), '%d %B, %Y')`"
fontsize: 20pt 
output:
  html_document:
    template: UserReport_template.html
    toc: true
...
# Sets up R markdown environment, reads input vector, names it
knitr::opts_chunk$set(echo = F)
inputArgs <- commandArgs(trailingOnly = T)
inputArgs <- c('TestUser', 'TestPI', 'NXT0570', 'nxid13446', 'BRB_AM_10',
               'danio_rerio', 'GRCz11.100_GFP', '../test_input/sampleTable.csv',
               '../test_input/theResult/fastQC/nxid13446/BRB_AM_10/BRB_AM_10_S7_R1_001_fastqc/fastqc_data.txt',
               '../test_input/theResult/fastQC/nxid13446/BRB_AM_10/BRB_AM_10_S7_R2_001_fastqc/fastqc_data.txt',
               '../test_input/theResult/trimmed/nxid13446/BRB_AM_10/NXT0570_nxid13446_BRB_AM_10_trimming_report.txt',
               '../test_input/theResult/trimmed/nxid13446/BRB_AM_10/BRB_AM_10_S7_R2_001_val_2_fastqc/fastqc_data.txt',
               '../test_input/theResult/demultiplexed/nxid13446/BRB_AM_10/stats.txt',
               '../test_input/theResult/mapStatsTab.csv',
               '../test_input/theResult/countTables/NXT0570_nxid13446_GRCz11.100_GFP_BRB_AM_10_readsCombined.csv')
names(inputArgs) <- c('User', 'PI', 'RunID', 'LibraryID', 'SampleID', 'Specie',
                      'Genome', 'submissionTab', 'fastqcR1', 'fastqcR2',
                      'trimInfoR2', 'fastqcTrimR2',
                      'demultStats', 'mapStats', 'countTab')
```

```{r setup_and_functions, include = FALSE}
# Loads all the R libraies and functions needed
...
colorCode <- c("mapped to mult. loci" = '#2B4C7E', 
               "NOT mapped - mismatches" = '#567EBB', 
               'NOT mapped - too short' = '#BEDCE7',
               'NOT mapped - other' = '#DCE0E6', 
               'mapped to too many loci' = '606D80',
               "uniquely mapped" = "#3B8686")
...
plotPerBaseQuality_ggplot2 <- function(dtToPlot, plotTitle = '') {
  # areas to color according to if they pass quality threshold or not
...
readInTrimmingInfo <- function(filePath) {
  # lines of interest
  LOI <- c('Sequence:', 'Total read pairs processed:', 
           'Read 2 with adapter:', 'Pairs written (passing filters):',
  ...
  for (oneLine in allLines){
    if (grepl(LOI, oneLine)) {
      result <- c(result, gsub(';.*', '', gsub('^ +', '', oneLine)))
    }
  }
  close(connect)
...
#' @param submissTab sabmission table with iser information
#' @return list with two elements: Overview and PercPerBarcode
readDemultiplexInfo <- function(filePath, submissTab) {
  # connection to the file
  connect <- file(filePath, open = "r")
  # read ALL lines of the file, luckily it's not many
  allLines <- readLines(connect)
  ...
  # change column names
  colnames(result[[2]])[1] <- 'SampleName'
  colnames(result[[2]])[2] <- 'Reads'
  # add row and column position in plate
  setkey(submissTab, 'SampleName')
  result[[2]]$PlateRow <- submissTab[result[[2]]$SampleName]$plateRow
  result[[2]]$PlateCol <- submissTab[result[[2]]$SampleName]$plateCol
...
plotDemultiplexPlate_plotly <- function(dtToPlot, plotTitle) {
  # template of the plate
  emptyPlate <- data.table(PlateRow = rep(LETTERS[1:8], each = 12), 
                           PlateCol = rep(1:12, 8))
  setkey(emptyPlate, PlateRow, PlateCol)
  dtToPlot_datTab <- data.table(dtToPlot)
  setkey(dtToPlot_datTab, PlateRow, PlateCol)
  filledPlate <- merge(emptyPlate, dtToPlot_datTab, all = T)
  
  fig <- plot_ly() %>% 
    add_trace(data = filledPlate, x = ~as.factor(PlateCol), y = ~PlateRow,
              type = 'scatter', mode = 'markers', 
              marker = list(color = ~Percent, colorscale = 'Viridis',
                            size = ~ifelse(is.na(Percent), 0, 15),
                            opacity = 1, colorbar = list(title = 'Colorbar')),
              text = ~paste0("Sample: ", SampleName, "\nValue: ",
                              round(Percent, 2)),
              hoverinfo = "text") %>%
    layout(title = plotTitle, 
           ...
           xaxis = append(alitheaPlotlyAxis,
                          list(title = '', 
                          tick0 = 1, dtick = 1, showgrid = T)),
           yaxis = append(alitheaPlotlyAxis,
                          list(title = '', tick0 = 1, dtick = 1,
                          autorange="reversed", showgrid = T)))
  fig
}

#' plotConcToReadCorr_plotly
#' Plots correlation between sample concentarion and number of demultiplexed 
#' reads
#' @param demultToPlot, data table to plot, result of 
#'                      readDemultiplexInfo(x)[[2]]
#' @param submissTab table with submission information from user
#' @param plotTitle title of the plot
#' @retun plotly
plotConcToReadCorr_plotly <- function(demultToPlot, submissTab, plotTitle) {
  # convert demultiplex statistics to data table and merge with 
  # submission table
  demultStatsDT <- as.data.table(demultToPlot)
  setkey(demultStatsDT, SampleName)
  setkey(submissTab, SampleName)
  dtToplot <- merge(demultStatsDT, submissTab, all = T)
  
  # linear regression
  fit <- lm(Reads ~ concentration, data = dtToplot)
  
  fig <- plot_ly(data = dtToplot) %>% 
  add_trace(x = ~concentration, y = ~Reads, type = "scatter", mode = "markers", 
            marker = list(color = basicColor),
            text = ~ paste0("# Reads: ", Reads,
                            "\n Conc.: ", concentration), 
            hoverinfo = "text") %>%
  add_lines(x = ~concentration, y = fitted(fit))  %>%
  add_text(x = ~concentration, y = ~Reads, text = ~SampleName,
           textposition = "top right") %>%
  layout(title = plotTitle, showlegend = F, font = alitheaPlotlyFont,
         xaxis = append(alitheaPlotlyAxis, list(title = 'Concentration')),
         yaxis = append(alitheaPlotlyAxis, list(title = 'Number of reads')))
  fig
}
...
readMapStatTab <- function(mappingStatsPath) {
  ...
  idColNames <- c('RunID', 'LibraryID', 'SampleID', 'Position', 'SampleName',
                  'Specie', 'Genome')
...
convertMapStatsToPerc <- function(tabMapStats) {
  ...
  idColNames <- c('RunID', 'LibraryID', 'SampleID', 'Position', 'SampleName',
                  'Specie', 'Genome')
  # select ID columns
  tabMapStatsPerc <- tabMapStats[, idColNames, with = F]
  # select columns with raw statistics values (integer)
  rawStats <- tabMapStats[, !colnames(tabMapStats) %in% idColNames, with = F]
...
createButtons <- function(dataList, statSortedBy, visibilityList) {
  changedYaxisTickLabs <- dataList[[statSortedBy]][[statSortedBy]]$SampleName
...
addAllBarChartTraces <- function(figToAdd, dataList, statSortedBy, plotColors,
  ...
  for (oneMapStat in otherStats) {
    dataToPlotOneTrace <- dataList[[statSortedBy]][[oneMapStat]]
    figToAdd <- figToAdd %>% 
                add_trace(data = dataToPlotOneTrace,
                          x = ~Value, y = ~SampleName, 
                          text = ~ paste0(SampleName, "\n", round(Value, 2)),
...
plotMappingStats_plotly <- function(dtToPlot, plotTitle) {
  ...
  for (i in 1:length(dtList)) {
    ...
    for (j in 1:length(dtList[[i]])) {
      colnames(dtList[[i]][[j]]) <- c('SampleName', 'Value')
    }
  }
...
calcPCA <- function(countData, mapStats){
  ...
  countsPCA$SampleName <- rownames(countsPCA)
  countsPCA <- as.data.table(countsPCA)
  mapStatsReduced <- mapStatTab[, c('SampleName', 'uniquely mapped')]
  setkey(countsPCA, SampleName)
  setkey(mapStatsReduced, SampleName)
...
plotPCA_plotly <- function(pcaListToPlot, plotTitle) {
  ...
  add_trace(x = ~PC1, y = ~PC2, type = "scatter", mode = "markers", 
            ...
            hoverinfo = "text") %>%
  add_text(x = ~PC1, y = ~PC2, text = ~SampleName,
           textposition = "top right") %>%
  layout(title = plotTitle, showlegend = F, font = alitheaPlotlyFont,
         xaxis = append(alitheaPlotlyAxis, 
...
# submission table which came from user
submissionTab <- fread(inputArgs['submissionTab'], header = T)
submissionTab <- submissionTab[RunID == inputArgs['RunID'] &
                               LibraryID == inputArgs['LibraryID']]
...
mapStatTab <- mapStatTab[RunID == inputArgs['RunID'] &
                         LibraryID == inputArgs['LibraryID'] &
                         SampleID == inputArgs['SampleID'] &
                         Genome == inputArgs['Genome']]

# count table 
countTab <- fread(inputArgs['countTab'])
# remove quality rows and gene ids/names
qcNames <- c("__alignment_not_unique", "__ambiguous", "__no_feature",
             "__not_aligned", "__too_low_aQual")
countTabNoQCnoGenes <- countTab[!Gene_id %in% qcNames, ]
countTabNoQCnoGenes <- countTabNoQCnoGenes[, -c('Gene_id', 'Gene_name')] 
```

```{r computeExonicReads, include = FALSE}
# calculate number of exonic reads
exonicReads <- colSums(countTabNoQCnoGenes)
exonicReads <- data.table(SampleName = names(exonicReads), Reads = exonicReads)

# compute percentage of exonic reads from uniquely mapped reads
mapStatTabUniqMap <- mapStatTab[, c('SampleName', 'uniquely mapped'), with = F]
mapStatTabUniqMap <- as.data.table(mapStatTabUniqMap)
# merge with exonic reads
setkey(exonicReads, 'SampleName')
setkey(mapStatTabUniqMap, 'SampleName')
...
numbExprGenes1 <- apply(countTabNoQCnoGenes, 2, function(x) sum(x > 1))
numbExprGenes1 <- data.frame(SampleName = names(numbExprGenes1), 
                             NumbGenes = numbExprGenes1)
# number of genes with > 10 counts
numbExprGenes10 <- apply(countTabNoQCnoGenes, 2, function(x) sum(x > 10))
numbExprGenes10 <- data.frame(SampleName = names(numbExprGenes10), 
                              NumbGenes = numbExprGenes10)
```

```{r integratedTable_1, include = F}
# Compile integrated table with sequencing statistics

integratedTable_1 <- submissionTab[,.N, by = .(RunID, LibraryID, SampleID)]
integratedTable_1 <- cbind(integratedTable_1, t(basicStats[, 1]), 
                           t(basicStats[, 2]))
colnames(integratedTable_1) <- c("RunID", "LibraryID", "SampleID", 
                                 "Number of samples", "R1: number of reads",
                                 "R1: length", "R1: GC%", 
                                 "R2: number of reads",
                                 "R2: length", "R2: GC%")
integratedTable_1_fileName <- paste(integratedTable_1$RunID, 
                                    integratedTable_1$LibraryID,
                                    integratedTable_1$SampleID, sep = '_')
write.table(integratedTable_1, 
            paste0(integratedTable_1_fileName, '_SeqStats.csv'),
            col.names = T, row.names = F, quote = F, sep = '\t', append = F)
```

```{r integratedTable_2, include = F}
# Compile integrated table with demultiplexing and mapping statistics

# first, merge demultiplexed stats and mapping stats
demultiplexStatsDT <- as.data.table(demultiplexStats[[2]])
setkey(demultiplexStatsDT, SampleName)
setkey(mapStatTab, SampleName)
integratedTable_2 <- merge(demultiplexStatsDT, mapStatTab, all = T)
setnames(integratedTable_2, 'Reads', 'Demultiplexed reads')
setnames(integratedTable_2, 'Percent', '% of library total reads')
integratedTable_2[, PlateRow := NULL]
integratedTable_2[, PlateCol := NULL]
integratedTable_2[, `total reads` := NULL]

# add number of exonic reads
exonicReadsDT <- as.data.table(exonicReads)
exonicReadsDT[, `uniquely mapped` := NULL]
colnames(exonicReadsDT) <- c('SampleName', 'Number of exonic reads', 
                             '% of exonic reads')
setkey(integratedTable_2, SampleName)
setkey(exonicReadsDT, SampleName)
integratedTable_2 <- merge(integratedTable_2, exonicReadsDT, all = T)

# add number of expressed genes, >1 reads
numbExprGenes1DT <- as.data.table(numbExprGenes1)
colnames(numbExprGenes1DT) <- c('SampleName', 'Number of genes with > 1 count')
setkey(integratedTable_2, 'SampleName')
setkey(numbExprGenes1DT, 'SampleName')
integratedTable_2 <- merge(integratedTable_2, numbExprGenes1DT, all = T)

# add number of expressed genes, >10 reads
numbExprGenes10DT <- as.data.table(numbExprGenes10)
colnames(numbExprGenes10DT) <- c('SampleName', 
                                 'Number of genes with > 10 count')
setkey(integratedTable_2, 'SampleName')
setkey(numbExprGenes10DT, 'SampleName')
integratedTable_2 <- merge(integratedTable_2, numbExprGenes10DT, all = T)

# write into table
integratedTable_2_fileName <- paste(unique(integratedTable_2$RunID),
                                    unique(integratedTable_2$LibraryID),
                                    unique(integratedTable_2$SampleID),
                                    unique(integratedTable_2$Genome),
                                    sep = '_')
write.table(integratedTable_2, 
            paste0(integratedTable_2_fileName, '_MapStats.csv'),
            col.names = T, row.names = F, quote = F, sep = '\t', append = F)
```
...
sampleNumb <- length(unique(demultiplexStats[[2]]$SampleName))
...
is 20 - 21 bp, and typical read 2 length is 65 - 75bp.
...
reads with adapters and quality trimmed reads. Some reads were removed due to
either bad quality of bases or adapter contamination. Row "Total written
(filtered)" gives a precise number and percentage of how many reads from the 
set were retained for further analysis. 
...
plotAdapterContent_plotly(adaptersTrim_R2, 'Adapter contamination in R2')
```
...
## Bar plot - values
The chart below depicts a distribution of demultiplexed reads per sample. A two
fold variation is expected and normal.

```{r demultiplexStatsValuesPlot, fig.height = mapPlotHeigth, fig.width = 8.5, fig.align = "left"}
plotBarChart_plotly(demultiplexStats[[2]], 'Reads', '# of reads per sample',
...
## Table
```{r demultiplexTab, results = 'asis'}
tabToShow <- demultiplexStats[[2]]
colnames(tabToShow) <- c('Sample name', '# of reads', "Percent", "Plate row",
                         'Plate column')
kable(tabToShow, caption = 'Demultiplexing statistics') %>%
...
## Correlation with sample concentration
This plot allows you to assess if any sample was undersequenced dut to the low
concentration
```{r corrConcReads, fig.height = 6, fig.width = 8.5, fig.align = "left", warning = F}
plotConcToReadCorr_plotly(demultiplexStats[[2]], submissionTab, 
                          'Correlation between sample concentation and number of reads') 
```
...
colnames(exonicReads) <- c('Sample', 'Number of uniquely mapped reads',
...
```{r pcaRaw, fig.height = 6, fig.width = 8.5, fig.align = "left", warning = F, message = F}
plotPCA_plotly(countsRawPCA, 'PCA based on raw counts')
```

## normalized data
PCA based on normalized for sequencing depth data
```{r PcaNorm, fig.height = 6, fig.width = 8.5, fig.align = "left", warning = F}
