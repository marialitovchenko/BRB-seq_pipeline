...
plotDemultipleStats <- function(dtToPlot, plotTitle) {
  # data frame to dysplay values ordered by percentage
  byPerc <- data.frame(x = dtToPlot$Barcode, y = dtToPlot$Percent, 
                       stringsAsFactors = F)
  byPerc <- byPerc[order(byPerc$y), ]
  byPerc$x <- factor(byPerc$x, levels = byPerc$x)

  # data frame to dysplay values ordered by name
  byName <- data.frame(y = dtToPlot$Percent, 
                       x = paste(dtToPlot$PlateRow, dtToPlot$PlateCol),
                       stringsAsFactors = F)
  byName <- byName[order(byName$x), ]
  byName$x <- factor(byName$x, levels = byName$x)
  ...
  fig <- plot_ly() %>% 
    ...
    layout(updatemenus = updatemenus, title = plotTitle, showlegend = F,
           xaxis = list(title = 'Sample', dtick = 1))
...
```{r demultiplexStatsPlot, fig.height = 8, fig.width = 14, fig.align = "left"}
plotDemultipleStats(demultiplexStats[[2]], '')
```