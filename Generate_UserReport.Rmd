...
plotDemultipleStats_plotly <- function(dtToPlot, plotTitle) {
  ...
  byName <- data.frame(x = dtToPlot$Percent, y = dtToPlot$Barcode,
                       stringsAsFactors = F)
...
plotMappingStats_plotly <- function(dtToPlot, plotTitle) {
  ...
  updatemenus <- list(list(active = -1, type = "buttons", 
                           buttons = buttonList))
...
## Plate view
```{r demultiplexPlatePlot, fig.height = 6, fig.width = 8, fig.align = "left"}
...
# Mapping
```{r mapStatsPlot, fig.height = 6, fig.width = 8, fig.align = "left"}
